	...
	<view class="w-100 h-100 d-flex">
		<evan-form class="evan-form-show flex-1 d-flex" :hide-required-asterisk="hideRequiredAsterisk" ref="form" :model="datas">
			<view class="w-100 h-100 d-flex flex-column">
				...
				<swiper :current="tabIndex" :duration="20" class="swiper-box" style="flex: 1;" @change="swiperChange">
					<!-- 客户分类 -->
					...
					<swiper-item class="swiper-item d-flex w-100  ">
						<scroll-view class="scroll-v flex-1 list" enableBackToTop="true" scroll-y>
							...
							<evan-form-item :label-style="labelStyle" label="门店名称" class="evan-form-item" prop="customer_name">
								<input
									type="text"
									confirm-type="确定"
							...
							<evan-form-item :label-style="labelStyle" label="商业单位名称" class="evan-form-item">
								<input
									type="text"
									confirm-type="确定"
									class="form-input mynowarp"
							...
							<evan-form-item :label-style="labelStyle" label="商业单位门店编码" class="evan-form-item">
								<input
									type="text"
									confirm-type="确定"
									class="form-input mynowarp"
							...
							<evan-form-item :label-style="labelStyle" label="联系人" class="evan-form-item" prop="contacts">
								<input
									type="text"
									confirm-type="确定"
									class="form-input mynowarp"
							...
							<evan-form-item :label-style="labelStyle" label="联系电话" class="evan-form-item" prop="mobile">
								<input
									type="number"
									confirm-type="确定"
									class="form-input mynowarp"
							...
							<evan-form-item :label-style="labelStyle" confirm-type="确定" label="详细地址" class="evan-form-item" prop="address">
							...
							<evan-form-item :label-style="labelStyle" label="门店所在地邮政编码" class="evan-form-item" prop="location_code">
								<input
									type="number"
									confirm-type="确定"
									class="form-input mynowarp"
					...
					<swiper-item class="swiper-item d-flex w-100  ">
						<scroll-view class="scroll-v flex-1 list" enableBackToTop="true" scroll-y>
							...
							<view class="d-flex a-center j-center mt-3">
								<view class="a-center-child" :class="zsindex == 0 && 'a-center-child-l'" @click="changeZindex(0)">三证合一</view>
								<view class="a-center-child" :class="zsindex == 1 && 'a-center-child-l'" @click="changeZindex(1)">三证</view>
							</view>
							<!-- 三证合一 -->
							<view class="form" v-if="zsindex === 0">
								...
								<evan-form-item :label-style="labelStyle" label="单位名称" class="evan-form-item" prop="company_name">
									<input
										type="text"
										confirm-type="确定"
										class="form-input mynowarp"
							...
							<evan-form-item :label-style="labelStyle" label="地址" class="evan-form-item" show_jiantou prop="consignee_area" @click="selectCity(2)">
								<view class="text-right w-100" :class="consignee_area_name ? 'text-666' : 'text-ccc'">{{ consignee_area_name || '请选择地区' }}</view>
							</evan-form-item>
...
export default {
	...
	onLoad(option) {
		...
		if (option) {
			...
			console.log(option);
			this.datas.is_active = option.is_active;
		}
		this.$nextTick(() => {
	...
	computed: {
		...mapState({
			...
			systemDict: state => state.channel.systemDict,
	...
	methods: {
		...mapActions(['getQuDaoData', 'getStoreData', 'OtherClass']),
		...
		/**
		 * @param {number} int  0==>保存  || 1==>提交
		 * @param {object}  reqData  创建一个空对象
		 * */
				...
				data &&
					this.holdpost(data, () => {
						setTimeout(function() {
							...
							uni.navigateBack({
								success() {
									beforePage.onPullDownRefresh({ tabIndex: 0 });
								}
							});
						}, 1500);
						uni.showToast({
				...
				data &&
					this.holdpost(data, () => {
						setTimeout(function() {
							...
							uni.navigateBack({
								success() {
									beforePage.onPullDownRefresh({ tabIndex: 1 });
								}
							});
						}, 1500);
						uni.showToast({
...
.l-form-item{border-bottom: 1rpx solid #eee;width: 750rpx;height: 88rpx;box-sizing: border-box;padding: 20rpx 16rpx 20rpx 36rpx;display: flex;align-items: center;
	 .l-item-label{
		 font-size: 30rpx;min-width: 130rpx;padding-right: 40rpx; flex-shrink: 0; color: #333333;
		  &::before{
