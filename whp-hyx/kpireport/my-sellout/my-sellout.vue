...
export default {
	...
	methods: {
		...
		getYearSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.sellout_record_amount,
					data: data,
					success: res => {
		...
		getBrandCategoryMonthSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.sellout_brand_record_amount,
					data: data,
					success: res => {
		...
		lookDetail(type,item){
			let obj = {};
			if(item.brand_pc_name == 'sellout汇总报表'){
				obj.navtitleText = `sellout${type === 1 ? '年' : '月'}汇总数据明细`;
				obj.url = this.$myhttpUrl.kpi_my_sellout_all_items;
			}else {
				obj.navtitleText = `${item.brand_pc_name}sellout${type === 1 ? '年汇总数据' : '月完成进度'}明细`;
				obj.url = this.$myhttpUrl.kpi_my_sellout_brand_pc_items;
			}
			obj.type = type;
