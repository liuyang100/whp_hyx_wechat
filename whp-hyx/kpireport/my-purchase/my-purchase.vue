<template>
	<view class="current-page">
		<channel-search :cust="resultData" :value-name="valueName" @search="search" @reset="reset" @selperson="selperson"></channel-search>
		<!-- 年汇总数据 -->
	...
	methods: {
		...
		async __init(data = {}) {
			...
			try {
				...
				// console.log(JSON.stringify(yearSummary,null,4));
				// const datas=	{
				// 	    "column": {
				// 	        "categories": [
				// 	            "1月",
				...
				// 	        ]
				// 	    },
				// 	    "brand_pc_name": "客户采购汇总报表",
				// 	    "task_amout_year": "660000.00",
				// 	    "amout_year": "42000.00"
				// 	}
		...
		lookDetail(type, item) {
			let obj = {};
			if (item.brand_pc_name == '客户采购汇总报表') {
				obj.navtitleText = `采购${type === 1 ? '年' : '月'}汇总数据明细`;
				obj.url = this.$myhttpUrl.kpi_mycg_all_items;
			} else {
				obj.navtitleText = `${item.brand_pc_name}采购${type === 1 ? '年汇总数据' : '月完成进度'}明细`;
				obj.url = this.$myhttpUrl.kpi_mycg_brand_pc_items;
			}
			obj.type = type;
		...
		getYearSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.kpi_mycg_all,
					data: data,
					success: res => {
		...
		getBrandCategoryMonthSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.kpi_mycg_brand_pc,
					data: data,
					success: res => {
