	...
	<view class="current-page">
		...
		<view class="card--box" v-for="(item, index) in list" :key="index">
			<template v-if="index === 0">
				<view class="y-head--style">{{ item.brand_pc_name }}</view>
				<checkbox-group @change="checkboxChange" class="d-flex a-center px-3">
					<checkbox class="uni-checkbox-input" :value="is_existed" :checked="is_existed" />
					是否含代理分销门店零售金额
				</checkbox-group>
			</template>
...
export default {
	...
	computed: {
		...
		reqParams() {
			...
			if (this.is_existed !== null) {
				temp.is_existed = this.is_existed;
			}
			return temp;
		},
		brandParams() {
	...
	methods: {
		...
		lookDetail(type, item) {
			let obj = {};
			if (item.brand_pc_name == '零售汇总报表') {
				obj.navtitleText = `零售${type === 1 ? '年' : '月'}汇总数据明细`;
				obj.url = this.$myhttpUrl.record_amount_items;
			} else {
				obj.navtitleText = `${item.brand_pc_name}零售${type === 1 ? '年汇总数据' : '月完成进度'}明细`;
				obj.url = this.$myhttpUrl.brand_record_amount_items;
			}
			obj.type = type;
			...
			obj.is_existed = this.is_existed;
		...
		checkboxChange(e) {
			if (this.is_lock) return;
			this.is_lock = true;
			if (this.is_existed === 0) {
				this.is_existed = 1;
			} else if (this.is_existed === 1) {
				this.is_existed = 0;
			}
			this.more_index = 0;
			this.loadingType = 5;
		...
		getYearSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.record_amount,
					data: data,
					success: res => {
		...
		getBrandCategoryMonthSummary(data = {}) {
			return new Promise((resolve, reject) => {
				this.$myRequest({
					url: this.$myhttpUrl.brand_record_amount,
					data: data,
					success: res => {
